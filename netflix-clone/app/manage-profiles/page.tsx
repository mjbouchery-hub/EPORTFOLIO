...
function AddProfileCard({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-48 w-32 flex-col items-center justify-center gap-3 text-white/70 transition-colors hover:text-white"
      aria-label="Add profile"
    >
      <span className="flex h-20 w-20 items-center justify-center rounded-full border-2 border-white/40 text-5xl font-light leading-none">
        +
      </span>
      <span className="text-sm font-semibold tracking-wide">Add Profile</span>
    </button>
  );
}
